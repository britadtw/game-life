...
export default function TasksPageClient({ goals, tasks, createTask }: Props) {
  const now = new Date();
  ...
  const dailyTasks = filteredTasks.filter(t => t.type === TaskType.DAILY);
  const weeklyTasks = filteredTasks.filter(t => t.type === TaskType.WEEKLY);
  const oneTimeTasks = filteredTasks.filter(t => t.type === TaskType.ONE_TIME);
      ...
      <div className="space-y-6">
        ...
        <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-lg border border-gray-200 dark:border-gray-700 p-6">
          ...
          {dailyTasks.length === 0 ? (
            <div className="text-center py-8 text-gray-400 dark:text-gray-500">
              <p>{selectedGoalId ? "此目標尚無每日任務" : "請先選擇一個目標"}</p>
            </div>
          ) : (
        ...
        <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-lg border border-gray-200 dark:border-gray-700 p-6">
          ...
          {weeklyTasks.length === 0 ? (
            <div className="text-center py-8 text-gray-400 dark:text-gray-500">
              <p>{selectedGoalId ? "此目標尚無每週任務" : "請先選擇一個目標"}</p>
            </div>
          ) : (
            <div className="space-y-3">
        ...
        <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-lg border border-gray-200 dark:border-gray-700 p-6">
          <div className="flex items-center gap-3 mb-6">
            ...
            <div>
              ...
              <p className="text-sm text-gray-500 dark:text-gray-400">目標期間只能完成一次</p>
            </div>
          </div>
          
          {oneTimeTasks.length === 0 ? (
            <div className="text-center py-8 text-gray-400 dark:text-gray-500">
              <p>{selectedGoalId ? "此目標尚無單次任務" : "請先選擇一個目標"}</p>
            </div>
          ) : (
            <div className="space-y-3">
              {oneTimeTasks.map((task) => (
                <div
                  key={task.id}
                  className="p-4 rounded-xl bg-gradient-to-r from-amber-50 to-orange-50 dark:from-amber-900/10 dark:to-orange-900/10 border border-amber-200 dark:border-amber-800"
                >
                  <div className="flex items-start justify-between mb-2">
                    <h3 className="font-semibold text-gray-900 dark:text-white">{task.title}</h3>
